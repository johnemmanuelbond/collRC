...
The :py:mod:`locality <calc.locality>` module processes particle positions to compute local environment metrics like nearest neighbors in 2D cartesian, 2D stretched, 2D projected, and 3D cartesian spaces.
...
The :py:mod:`bond_order <calc.bond_order>` module computes complex-valued bond orientational order in 2D cartesian, 2D stretched, 2D projected, and 3D cartesian spaces (via steinhardt calculations).
