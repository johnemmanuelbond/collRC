...
def global_patic(angles:np.ndarray, p:int=2) -> np.complexfloating:
    ...
    .. math::
        ...
        P_p = \\langle e^{i p \\theta_j}\\rangle = |P_p|e^{i p \\theta_p}.

    :param angles: an (N,) array of orientations of each particle in the frame
    :type angles: ndarray
...
def local_patic(angles:np.ndarray, nei_bool:np.ndarray, p:int =2) -> np.ndarray:
    ...
    :param angles: an (N,) array of orientations of each particle in the frame
    :type angles: ndarray
    :param nei_bool: a (N,N) boolean array defining particle neighbors
